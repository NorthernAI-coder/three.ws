...
const phase = await page.waitForFunction(() => window.__CC__?.phase === 'world', { timeout: 45000 }).then(() => 'world').catch(() => 'TIMEOUT');
...
const r = await page.evaluate(() => {
	...
	return {
		...
		joyZ: cs(joy)?.zIndex,
		hitAtJoystick: hit(joy),                 // want #cc-joystick
		hitAtPrimaryBtn: primaryBtn ? hit(primaryBtn) : 'no-btn', // want something inside the card
...
console.log('hit @ joystick centre :', r.hitAtJoystick, '(want #cc-joystick — backdrop is click-through)');
console.log('hit @ primary button  :', r.hitAtPrimaryBtn, '/ inCard:', r.primaryBtnInCard, '(card still interactive)');

const pass = r.overlayPE === 'none' && r.cardPE === 'auto' && r.joyZ === '60' && r.hitAtJoystick === '#cc-joystick' && r.primaryBtnInCard;
console.log('\nRESULT:', pass ? 'PASS — joystick reachable during onboarding, card still works' : 'FAIL');
await browser.close();
