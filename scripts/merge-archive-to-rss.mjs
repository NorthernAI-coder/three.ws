#!/usr/bin/env node
// Merge tweets from a data/archives/<account>_tweets_*.json file into
// data/rss/items.json WITHOUT overwriting existing curated entries.
// Only adds new items (by id). Existing items are preserved exactly.
// Usage: node scripts/merge-archive-to-rss.mjs [--account=trythreews] [--min=50]
...
const MIN_LEN = Number(args.min ?? 50);
...
console.log(`Existing items: ${existing.items.length}`);
...
// Merge: new entries inserted in chronological order among existing items
const merged = [...existing.items, ...newEntries].sort(
...
const out = {
	...
	_readme:
		...
		'Items are served newest-first by date; cap is 80. ' +
		`Last merged from data/archives/${newest} on ${new Date().toISOString().slice(0, 10)} ` +
		`(${merged.length} total items, ${newEntries.length} added).`,
	items: merged,
};

await writeFile(ITEMS_FILE, JSON.stringify(out, null, 2) + '\n', 'utf8');
console.log(`Wrote ${merged.length} total items to ${ITEMS_FILE}`);
