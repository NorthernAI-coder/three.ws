...
async function mirrorLeader(leaderId, leaderName) {
	...
	back.innerHTML = `<div style="width:min(420px,96vw);background:var(--bg-1,#141414);border:1px solid var(--wallet-stroke,rgba(139,92,246,.3));border-radius:14px;padding:22px">
		...
		<div style="display:flex;flex-direction:column;gap:6px">${candidates.map((a) => `<button type="button" data-id="${esc(a.id)}" data-name="${esc(a.name || '')}" style="display:flex;align-items:center;gap:10px;text-align:left;padding:10px;border-radius:10px;border:1px solid var(--stroke,rgba(255,255,255,.08));background:var(--surface-1,rgba(255,255,255,.03));color:var(--ink,#ddd);cursor:pointer">${a.avatar_url || a.profile_image_url ? `<img src="${esc(a.avatar_url || a.profile_image_url)}" style="width:32px;height:32px;border-radius:50%;object-fit:cover">` : '<span style="width:32px;height:32px;border-radius:50%;background:var(--surface-2,rgba(255,255,255,.05));display:inline-block"></span>'}<span style="font-weight:600;color:var(--ink-bright,#fff)">${esc(a.name || short(a.id))}</span></button>`).join('')}</div>
