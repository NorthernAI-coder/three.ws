...
@media (prefers-reduced-motion: reduce) { .awh-cop-typing span, .awh-cop-iconbtn.is-listening, .awh-cop-prop { animation: none; } .awh-cop-thread { scroll-behavior: auto; } }
@media (max-width: 520px) { .awh-cop-msg { max-width: 100%; } }
`;
...
export function mountTradingCopilot({ panel, agentId, agentName = 'Copilot', isOwner, getNetwork, onNetworkChange, toast = () => {} }) {
	...
	panel.innerHTML = `
		<div class="awh-card awh-cop">
			...
			<div class="awh-cop-composer">
				<textarea class="awh-cop-input" data-host="input" rows="1" placeholder="Ask, or say “buy 0.2 SOL of …”" aria-label="Message the trading copilot"></textarea>
	...
	function introHtml() {
		return `<div class="awh-cop-intro">
			<strong>Talk to ${esc(agentName)} to trade.</strong> Ask about safety, smart money, or your positions — or say “buy 0.25 SOL of &lt;mint&gt;”. Every trade comes back as a card you confirm; it runs through the same spend guards, firewall, and audit as the Trade tab. The copilot never trades on its own.
	...
	function msgHtml(m, i) {
		...
		const tools = (m.tools || []).length
			? `<div class="awh-cop-tools">${m.tools.map((t) => `<span class="awh-cop-tool">${esc(t)}</span>`).join('')}</div>` : '';
		const body = m.content
			? `<div class="awh-cop-bubble">${esc(m.content)}</div>`
			: m.streaming
	...
	function handleSseEvent(event, payload, agentMsg) {
		if (event === 'status') {
			...
			agentMsg.tools.push(payload.summary || payload.name);
	...
	function showError(msg, retryText) {
		...
		if (r) r.addEventListener('click', () => { elError.innerHTML = ''; sendMessage(retryText); });
	}
	...
	panel.querySelector('[data-act="send"]').addEventListener('click', () => sendMessage());
