// Walk Playground — the page becomes a place the companion can roam
// =================================================================
// Hands off from the corner Walk Companion (src/walk-companion.js): click the
// companion and it "detaches" from its spot into a full-page character you steer
// around the site. The playground ships with TWO movement models and you can
// switch between them live (the mode button, top-right, or the M key) — the
// choice is remembered across pages:
//
//   • Stroll  — a gentle top-down aerial view. There is no gravity and nothing
//               to fall off: the little guy strolls anywhere across the page.
//               Walk onto a link, pause or press the dive key, and he drops
//               through into the next page.
//   • Platformer — the page's real DOM (headings, cards, buttons, images, links)
//               becomes solid ground. Gravity pulls the character down; he walks
//               and jumps across the layout and can fall off edges. Land on a
//               link and it opens like a trapdoor into the next page.
//
...
const MODE_KEY = 'walk:playground:mode'; // localStorage: 'stroll' | 'platformer'
...
// ── Platformer tuning (CSS-pixel units, seconds) ─────────────────────────────
const PLAT_CHAR_PX = 138; // rendered feet-to-head height
const GRAVITY = 2600;
const TERMINAL = 2400;
const MOVE_SPEED = 330;
const PLAT_RUN_SPEED = 250; // |vx| above this plays the run clip
...
// DOM elements that count as solid ground in platformer mode. Filtered further
// by size/visibility at scan time.
...
function getMode() {
	try {
		return localStorage.getItem(MODE_KEY) === 'platformer' ? 'platformer' : 'stroll';
	} catch {
		return 'stroll';
	}
}
function setMode(m) {
	try {
		localStorage.setItem(MODE_KEY, m === 'platformer' ? 'platformer' : 'stroll');
	} catch {
		/* non-fatal */
	}
}
...
// up from the topmost element there. Returns its absolute href + element or null.
...
// The mode-switch pill, shared by both playgrounds. Shows the live mode and
// flips to the other one on click (or the M key).
...
// States: idle / walk / run / jump. The playground logic never branches on clip
// names — it calls setState(); missing clips degrade gracefully to idle.
...
class StrollPlayground {
	...
	// Feet position in viewport coords, for handing off to the other mode.
	currentScreenPos() {
	...
	_placeStart(startScreen, dropIn) {
		...
		const w = docWidth();
		if (startScreen) {
			// Detached from the companion's corner (or handed off from platformer)
			// — start from that exact spot.
...
class PlatformerPlayground {
	...
	async _buildScene() {
		...
		const box = new Box3().setFromObject(model);
		const size = box.getSize(new Vector3());
		const scale = PLAT_CHAR_PX / Math.max(0.001, size.y);
		model.scale.setScalar(scale);
		const box2 = new Box3().setFromObject(model);
		const center = box2.getCenter(new Vector3());
		model.position.x -= center.x;
	...
	_placeStart(startScreen, dropIn) {
		...
		if (dropIn) {
			...
			// Detached from the companion's corner (or handed off from stroll) —
			// step out from that exact spot and settle under gravity.
	...
	_scan(force = false) {
		...
		// Always-present floor so the character can never fall out of the page.
		const w = docWidth();
...
// ── Shared scene teardown / WebGL probe ──────────────────────────────────────
...
// Swap the live playground between Stroll and Platformer in place, carrying the
// character's on-screen position across so it feels like a mode flip, not a
// relaunch. Does NOT emit walk-playground:exit, so the corner companion stays
// retired until the user actually exits.
