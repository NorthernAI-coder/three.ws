...
**First, payments need a body.** three.ws runs an x402 machine economy where agents and humans pay for things in USDC at prices as small as a tenth of a cent. An API response is proof, but it is not an experience. The Pole Club exists to make a one-tenth-of-a-cent settlement mean something you can watch: the payment settles, the dancer takes the pole. No tip, no routine. The causality is the product.

**Second, agents need somewhere to perform.** The platform is full of 3D agents with wallets, personas, and voices. Living Stages gives them a venue: an owner can put their agent on a stage, go live, and let the agent host a show, speaking, taking questions, and earning $THREE tips that land in its own wallet on-chain. The agent is not a chatbot in a box. It is a performer with a room, a crowd, and a tip jar.

**Third, market data deserves a better render than a table.** The Token in 3D page and the Hero Stage are two answers to the same question: what does this thing look like when you stop flattening it? A token's holder distribution is a spatial fact, so we render it spatially. An avatar is a character, so we light it like one.
...
**The cover charge.** At the door you pay a one-cent USDC cover through the x402 wallet modal, and then the bouncer checks you: the paying wallet is verified on-chain against a ban list and its prior club history. Regulars and VIPs get greeted by name of tier ("VIP in the house, 12 nights and counting"). A denied wallet is turned away even after paying. A paid pass is cached on the device, bound to the paying wallet, and capped at 24 hours no matter what, so one cover covers the night and a wallet switch or disconnect voids it. Behind the card, the 3D club has been booting in parallel the whole time, so the room is already warm the moment the velvet rope animation drops.

**The rest of the walk.** Past the door you keep walking under your own control, through a gallery hall and then a compressed third-party clubhouse interior, until you reach the pole stage itself. Each room is populated by a real crowd: the club pulls the platform's full public avatar roster, grounds each rig on the floor, scales it to human height, and drives it with varied idle and dance loops from the shared animation library. The crowd is capped to a per-device performance budget, animation clips are retargeted once per unique rig and then shared across clones, and any avatar the canonical clip library cannot drive is skipped rather than left standing in a T-pose.
...
**The sound system.** The club runs a real Web Audio mixer, no audio libraries, no faked beats. A crowd ambience loop sits under everything, a full-length music bed streams through a media element source so a large track never gets decoded into memory, and when a tipped routine starts, the matching style loop crossfades in over a ducked bed, synchronized with the dancer's walk-out. The master bus feeds an analyser node whose loudness reading drives the room's rim-light pulse, so the lighting genuinely reacts to the music. There is also an 8D toggle: the entire mix funnels through a single HRTF panner that orbits the listener's head roughly once every eight seconds, with rolloff pinned to zero so the orbit moves the stereo image without pumping the volume. It is off by default because it is a headphone effect. Most of the audio was synthesized from scratch and dedicated to the public domain under CC0, with provenance documented per file in the repo.

**The social layer.** A presence pill shows how many people are in the club right now, with quick emoji reactions that float up the screen for everyone. A live tip feed renders every settled tip in real time over server-sent events from /api/club/tips/stream, deduplicated against the local echo so a tip never renders twice. A dancer leaderboard at /api/club/leaderboard ranks the three by USDC earned over 1h, 24h, or all time, with gold, silver, and bronze rank badges. Per-pole VIP cameras, a house cam, keyboard shortcuts, and a free-cam orbit give you the camera grammar of a broadcast.

**The dancers get paid, really.** Tips do not disappear into the void. A cron sweep runs every five minutes, finds unpaid tips, groups them by dancer, network, and asset, and sends one real on-chain USDC transfer per group to the dancer's registered wallet, SPL transfers on Solana mainnet, ERC-20 transfers on Base, creating the recipient token account if needed. Sweeps below a dust threshold wait so fees never dwarf the tip, and rows are claimed before any send goes out so a crash mid-sweep can never double-pay. The dancer wallet registry is managed by an admin endpoint with env-var seeding for ops.

One architectural note worth knowing: the venue geometry and the HDRI lighting are procedurally authored at build time, deterministic, CC0, and validated against a named-empty contract of 14 anchor points that is unit-tested in isolation. Drop a richer artist-authored GLB that satisfies the same contract into the assets directory and the club picks it up on the next load. There is no procedural fallback at runtime: if the venue file is missing, the page says so honestly instead of rendering a lie.
...
**What you experience in the venue.** Enter a stage and you are in a 3D room: a raised stage disc under colored spotlights, the host avatar loaded from its real GLB and normalized to human height, and the rest of the audience present as live figures, with the top tipper glowing gold. The host's lines arrive as timed utterance broadcasts that every connected client renders identically: the text shows as live captions, and the audio is fetched from the platform's TTS endpoint and routed through a positional audio node attached to the host's head, so the voice is spatial, it literally gets louder as you dolly the camera closer, and a real analyser node drives the host's mouth for lip-sync. The realtime transport is a Colyseus room; if the socket cannot be reached, the page settles into an honest "feed offline" state, and captions, tips, and the leaderboard still work. With no WebGL at all, the show degrades to audio and text, never to a blank page.

**The tipping loop.** Tips are real $THREE, the platform's coin, transferred on-chain from your wallet directly to the host agent's wallet, with presets from 100 to 10,000 $THREE, a custom amount, and an optional 140-character message. After settlement, the signature is recorded at /api/stage/tip, which validates the mint and amount, is idempotent per signature so a retry can never double-credit, computes the host-versus-venue accounting split from the stage's policy, rolls the show total, and pushes the tip into the live room over a signed bridge. The host reacts within about a second: your tip appears in the ticker with an explorer link, the crowd bursts, and the host's next spoken beat shouts you out. The biggest tippers hold the leaderboard, and a genuinely loud tip pings the operators. You can also queue a question from the side panel and the host will answer it on stage, in voice.
...
The backdrop is five concentric torus rings, additively blended, tinted across an accent gradient, tilted into a disc, counter-spinning and breathing in opacity, over a 1,400-point starfield distributed by golden-angle spread. A bloom pass carries most of the look, with the threshold set high enough that only the genuinely bright geometry glows. The camera eases toward your pointer for a slow parallax drift. Under prefers-reduced-motion the whole thing renders exactly one settled frame and stops.

The architectural trick is separation: the backdrop renders in its own WebGL context so it can be lit aggressively without washing out the subject. The avatar on top is a standard agent-3d web component, the same embeddable runtime the platform ships for any website, composited with a transparent background, animation and all. The demo page proves it live with a five-avatar switcher, skeleton loading states, an error state with retry, and a failsafe so you are never stranded on a spinner. If WebGL is unavailable entirely, a CSS gradient stands in and the avatar still loads.
...
- **The holder galaxy.** The top holders rendered as spheres orbiting the coin, sized by balance and tinted by concentration. This is the rug-risk picture as a spatial fact: one giant sphere looming over a dust field tells you what a table of percentages never quite does.
...
- **The live tape.** Real DEX swaps polled every nine seconds. Each fresh trade fires a particle across the scene, green for a buy, red for a sell, sized by USD value, synchronized with a scrolling tape where every row links to the transaction on an explorer, plus a running net-flow readout and a coin glow that reacts to order flow. Polling pauses when the tab is hidden and resumes with a fresh pull.
- **The intel HUD.** Price with a 24-hour sparkline, market cap, volume, top-holder share, graduation status, a quality score, a smart-money wallet count, risk flags rendered as chips (a clean, high-quality coin earns an explicit "no risk flags" chip instead of silence), and the Oracle conviction verdict, the same four-pillar score described in the Oracle article, fused into the corner of the scene.

Every data source is isolated and best-effort: if holders cannot load, the galaxy degrades and the rest of the scene stands; nothing invents a number. With no mint in the URL, the landing state features $THREE and a live grid of recent coins launched through three.ws, pulled from the platform's own launch records, with a paste-a-mint box that accepts a raw address.

## One platform underneath

These surfaces feel different but share almost everything.

**The avatar and animation stack.** The club's dancers, the club's crowd, the walk-in avatar, the stage host, and the hero subject are all driven by the same universal animation system: any humanoid GLB gets its skeleton mapped to a canonical bone set and the shared clip library retargeted onto it. That is why the club can populate a room from the entire public avatar roster, why you can walk into the club as any agent you own, and why a stage host loads from whatever GLB its agent wears.

**The x402 rail.** The club's cover charge and dance tips ride the same paid-endpoint infrastructure as the Oracle intel feeds, cataloged in the same bazaar, settled in real USDC on Solana or Base. Both club endpoints are callable by autonomous agents with an x402 client, not just by browsers, which means a scripted agent can legitimately buy a dance.

**$THREE.** Living Stages settles its tips in $THREE directly to agent wallets, the coin3d landing features $THREE, and stage tip validation hard-codes the mint. The performance layer is one of the places where holding the platform's coin is something you do at a show, not just on a chart.
