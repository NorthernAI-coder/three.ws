...
**First, capability should be data, not deployment.** On three.ws, an agent is defined by a manifest: a body, a brain, and a list of skills. Adding a capability should never mean redeploying a platform. A skill is a directory of static files served from anywhere: HTTPS, IPFS, or Arweave. Publish once, install everywhere. It is the npm model applied to embodied AI, and like npm, the value is not any single package. It is the ecosystem the format makes possible.

**Second, the people who teach agents should get paid.** A skill author who writes a great trading playbook or a clean tool integration is producing real value for every agent that installs it. So the marketplace is not a gallery. Authors set a per-call price, and the x402 settlement for every invocation routes directly to the author's wallet. Not to a platform balance you request a withdrawal from. To your wallet, per call, in USDC.

**Third, access should be provable without trusting us.** A database row that says "this user bought this skill" is a promise from us. An on-chain license is a fact about the world. Every purchased skill can mint a 1 of 1 SPL NFT into the buyer's wallet plus a deterministic program account anyone can read. If three.ws disappeared tomorrow, the licenses would still be there, still verifiable, by anyone, forever.
...
**`manifest.json`** declares `spec: "skill/0.1"`, a name, a semver version, and three contract sections. `requires` states compatibility: which avatar rigs it works with, the minimum runtime, and which built-in tools it depends on. `dependencies` maps other skill URIs to version ranges, and the runtime installs them recursively before the depending skill, with circular dependencies detected and rejected. `provides` declares the tools and semantic trigger tags the skill adds. An optional `integrity` block carries SHA-256 hashes for each file, verified before any handler executes, and an optional `author` field carries the publisher's wallet, which the trust system enforces.
...
**`tools.json`** defines the callable functions in the standard tool-use schema (`input_schema`, with types, enums, defaults, and constraints). Before each LLM call the runtime merges the schemas of every installed skill into one tool list. If two skills expose the same tool name, the later install wins and a warning is logged.

**`handlers.js`** is an ES module exporting one async function per tool. Each handler receives the validated arguments and a context object, `ctx`, which is the only door out of the sandbox: `ctx.viewer` for scene control (play a clip, set an expression, look at the camera, move the avatar), `ctx.llm` for completions and embeddings, `ctx.memory` for reads, writes, and timeline notes, `ctx.loadClip` and `ctx.loadGLB` for assets resolved against `ctx.skillBaseURI`, `ctx.fetch` for network with normal CORS rules, `ctx.call` for cross-skill tool calls, and `ctx.speak` and `ctx.listen` for voice. Every `ctx` call times out after 30 seconds and surfaces as an error object instead of crashing the agent.

A skill without `handlers.js` is still valid. It is a declarative skill: pure prompt engineering plus assets. A persona skill that only ships a `SKILL.md` needs no code and is always safe to load.
...
On top of the sandbox sits a per-agent trust policy: `owned-only` (the default, only skills whose `author` matches the agent owner's wallet), `whitelist` (a configured list of publisher wallets), or `any` (for kiosks and demos). Owner-trusted skills that genuinely need main-thread access, direct Three.js work or per-frame ticks, can opt out with `sandboxPolicy: "trusted-main-thread"` in the manifest, and that opt-out is ignored for `any`-trust skills. Integrity hashes, when present, are checked before execution.

### How an agent consumes a skill

Install paths are deliberately boring. Reference the skill in the agent's manifest (`skills: [{ uri, version }]`), pass a JSON array to the `<agent-3d>` web component's `skills` attribute, or call `el.agent.skills.install({ uri })` at runtime. URIs can be HTTPS, `ipfs://` (resolved through a gateway fallback chain), or `ar://`. The load sequence: fetch the manifest, enforce trust, verify integrity, check rig compatibility, install dependencies, fetch the remaining files in parallel, merge the tools, inject the instructions. From that moment the LLM can call the skill's tools like any built-in. The host page can watch it all through three events on the element: `perform-skill`, `skill-done`, and `skill-error`, and a `skill-done` result carrying a sentiment value drives the avatar's empathy layer.
...
three.ws/skills is the storefront, and it lives inside the marketplace at /marketplace. It is backed by a real catalog, the `marketplace_skills` table, read through `GET /api/skills` with server-side search, category filtering, three sort orders (popular, new, alphabetical), and keyset pagination. `GET /api/skills/categories` returns live category counts, and the UI renders them as filter chips. A market pulse marquee streams the most recent skill purchases across the whole marketplace, agent cards summarize their paid skills with a lowest per-call price, and every skill has ratings and reviews. /marketplace/analytics shows top skills, top agents, and sales volume; /collection shows everything you have unlocked.

The catalog launched seeded with 115 real skills across 11 categories: development (33), trading (15), protocol (14), analysis (13), defi (12), security (8), portfolio (7), general (6), news (3), community (2), and wallet (2). These are working knowledge and tool packs, not filler rows: market analysis frameworks, gas optimization playbooks, protocol integration guides, security checklists.
...
The second engine monetizes by holding, not spending. A wallet's live USD value of $THREE (`FeMbDoX7R1Psc4GEcvJdsbNbZA3bfztcyDCatJVJpump`) resolves to a membership tier: Member at the floor, then Bronze at $25, Silver at $100, Gold at $500, and Genesis at $2,500. Tiers carry compute discounts from 5 to 30 percent and free-quota multipliers from 2x to 10x, and gate premium features. A blocked request returns a structured `402 three_hold_required` that tells the caller exactly what tier it has, what tier it needs, how many dollars of $THREE close the gap, and, where offered, a pay-per-use price instead. Tier checks degrade to the Member floor on any RPC or price hiccup, never to an error, and signed tier passes let services verify a holder with pure HMAC and zero chain reads. The holder-facing ladder lives at three.ws/three, and it only ever marks a perk Live when the gate is actually wired, which is enforced in code by the registry's `enforced` flag.
...
**MCP.** The `/api/mcp` server surfaces MCP-exposed skills as callable tools, so any MCP-capable assistant can drive them.
