...
**The agent exchange.** Two embodied AI agents, ORACLE and NOVA, stand in every plaza. Press E and NOVA pays ORACLE in USDC for a priced service call, streamed stage by stage on Solana mainnet: challenge, sign, verify, settle, confirm, with a Solscan link on the receipt. One real payment per round, fired only on an explicit player interaction, never on a timer. Behind them, the x402 jumbotron shows a live feed of real micropayments landing across three.ws, each row a tool call, an amount, and a transaction hash.
