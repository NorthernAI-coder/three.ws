...
<div class="bg-paper min-h-screen">
  <div class="max-w-[1100px] mx-auto px-6 pt-24 pb-32">
    ...
    <!-- Eyebrow -->
    {#if content.eyebrow}
      <p class="text-center text-xs font-semibold uppercase tracking-[0.14em] text-ink-faint mb-4">
        {content.eyebrow}
      </p>
    {/if}
    ...
    <!-- Sub -->
    <p class="text-ink-soft text-lg max-w-[640px] mx-auto text-center mt-6">
      {content.sub}
    </p>
    ...
    <div class="mt-20 grid grid-cols-3 md:grid-cols-6 gap-6 items-center opacity-70">
      {#each logos as logo}
        <div class="text-ink-soft text-sm uppercase tracking-[0.16em] text-center">
          {logo}
        </div>
      {/each}
    </div>
