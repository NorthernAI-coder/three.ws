...
//      viem LocalAccounts and any compatible signer satisfy this.
...
async function resolveAddress(wallet) {
	...
	// Pre-built wallet object (viem LocalAccount / viem WalletClient / compatible signer).
	...
	const signTypedData = async (typedData) => {
		try {
			...
			if (typeof wallet.signTypedData === 'function') {
				// viem accounts take the typed-data object directly; any compatible
				// signer uses the same shape. Both resolve to a hex signature.
