...
// $THREE on Solana mainnet and is IRREVERSIBLE — it requires confirm:true (unless
// the owner durably pre-authorized it) and is capped by the daily $THREE budget.
...
export const def = {
	...
	// EXECUTION: a wallet_transfer proposal moves real $THREE on mainnet, irreversible.
	annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: true },
	description:
		'EXECUTE a proposal for real, within the owner-granted scope. ⚠️ This is the funds-touching action: a ' +
		'`wallet_transfer` proposal sends REAL $THREE on Solana mainnet and is IRREVERSIBLE — you MUST pass ' +
		'confirm:true for it (unless the owner durably pre-authorized that scope), and it is capped by the ' +
		'daily $THREE budget. `create_alert` and `briefing` are real writes but reversible via undo_action. ' +
	...
	inputSchema: {
		...
		confirm: z
			.boolean()
			.optional()
			.describe('Must be true to execute an irreversible $THREE wallet_transfer (when require_confirm is on). Ignored for reversible kinds.'),
	},
	async handler(args) {
