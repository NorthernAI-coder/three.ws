...
const lamToSol = (l) => (l == null ? 0 : Number(BigInt(l)) / LAMPORTS_PER_SOL);
const clamp = (x, lo, hi) => Math.min(hi, Math.max(lo, x));
const DEFAULT_MIRROR_SLIPPAGE_BPS = 300;

// --- Edge-score tuning. Transparent, named, tunable (cf. trader-stats). --------
const EDGE_WEIGHTS = { hitRate: 0.55, roi: 0.45 };
const EDGE_CONFIDENCE_FULL_AT = 10;  // closed signals for full statistical confidence
const EDGE_NEUTRAL = 0.45;           // unproven feeds regress toward this (slightly below mid)
const ROI_TANH_PCT = 50;             // ~+50% avg realized ≈ a strongly positive ROI sub-score
const FLAT_OUTCOME_PCT = 1;          // |realized| ≤ this counts as flat, not win/loss
const DUST_ORDER_SOL = 0.002;        // mirror orders below this are skipped as dust
