...
- Paid lanes already exist: paid MCP tools (USDC via x402, prompt `23`), skill
  ...
  `15`/`08`). Monetization design work: `prompts/monetization/`.
...
7. **Experiment:** price points and packaging are configurable (not hardcoded across
   the UI) so they can be tested; instrument upgrade funnel + LTV inputs (prompt `33`).

## Must-not
- Do not hardcode prices in scattered UI strings — source from one config.
