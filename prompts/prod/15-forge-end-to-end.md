...
**Owns:** `pages/forge.html`, forge API lane (text/image → 3D), `forge_free`/`mesh_forge`/`rig_mesh`/`forge_avatar` MCP tools, viewer (`avatar-sdk/`).
...
2. **Generation lifecycle:** real progress tied to the actual job (no fake bars), with
   ...
   jobs, timeouts, and partial failures with retry — never a hung spinner.
3. **Preview:** orbit, zoom, AR, and lighting all work; mobile touch-orbit smooth;
   poster/skeleton until model loads (prompt `10`); model framed nicely by default.
...
5. **Output actions:** download GLB, view in AR, open in viewer, save to account, and
   share (with a real OG image of the model) — every button does real work.
