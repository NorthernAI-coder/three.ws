...
**Owns:** `marketplace/`, `pages/marketplace*.html`, related `api/` (skills, listings, purchases), `prompts/monetization/` outcomes.
...
## Map
- `marketplace/` dir + `pages/` marketplace surfaces. Skill purchase flow and
  on-chain skill licenses: `api/skills/*`, `api/_lib/skill-license-onchain.js`,
...
3. **Purchase flow:** end-to-end with real payments (x402 / Solana Pay, prompt `18`).
   ...
   server-side price (prompt `07`). On success, the buy button becomes "Owned" and the
   skill/asset is actually usable.
4. **Ownership & licenses:** reflect on-chain `SkillLicense` state truthfully; gate
   access by the real license, handle pending/confirmed/failed clearly.
