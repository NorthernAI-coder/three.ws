...
**Owns:** `pages/agent-*.html` (detail, economy, exchange, trade, studio, wallet, embed), `api/agents/*`, agent identity/reputation (`contracts/`, ERC-8004), AgenC reads.
...
- Backend: `api/agents/*` (incl. `fork.js`), `api/_lib/avatar-agent.js`,
  `api/_lib/agent-wallet.js`, `solana-wallet.js`, `avatar-wallet.js`. Ownership model
  is in `prompts/agent-wallets/00-README-orchestration.md` (one agent = one owner,
