...
**Owns:** `mcp-server/`, `mcp-bridge/`, `packages/*-mcp/` (avatar-agent, pumpfun, ibm-watsonx, ibm-x402, three-token, threews-avatar), the 3d-agent MCP tool surface (`api/_lib/mcp-dispatch.js`, `mcp-error-sanitize.js`).
...
- Servers: `mcp-server/` (`@three-ws/mcp-server`), `mcp-bridge/`, and the
  `packages/*-mcp/` set. Dispatch/error libs: `api/_lib/mcp-dispatch.js`,
  `mcp-error-sanitize.js`. Tooling: `npm run audit:mcp`, `smoke:mcp`, `test:mcp`,
  `publish:mcp`(`:dry`).
...
1. **Spec conformance:** every server speaks current MCP transport correctly; each
   ...
   schema with validation. Errors return proper MCP error shapes via
   `mcp-error-sanitize.js`, never crashes.
...
4. **Discoverability:** tools/servers are discoverable (bazaar/x402 indexing, memory
   `x402-discovery-indexing`); a public catalog page lists them with prices and
   examples.
