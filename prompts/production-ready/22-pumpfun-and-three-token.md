...
**Owns:** `api/pump/*`, `pump-fun-skills/`, `packages/pumpfun-mcp/`, `packages/three-token-mcp/`, `pages/bulk-launch.html`, launch feeds, $THREE surfaces, sniper/oracle workers (`workers/`).
...
- Workers: `worker:oracle`, `worker:sniper` (`workers/`), deploy via `deploy:sniper`;
  `npm run pump:smoke`, `pump:refresh-idls`.
...
6. **Workers:** oracle + sniper run reliably with the resilience patterns (timeouts,
   retries, circuit-breakers via `api/_lib/resilience.js`), proper logging/alerting
   (prompt `25`), and safe shutdown. Never blind-retry trades (idempotency, prompt
   `07`). `pump:smoke` passes.
...
- [ ] Oracle + sniper workers resilient, logged/alerted, idempotent; `pump:smoke` passes.
