...
## Map
- Build: `vite.config.js`, scripts `build`, `build:fast`, `optimize:glb`,
  `compress:glbs`. GLB/CDN assets served via R2 (`npm run apply:r2-cors`).
- Heavy modules: Three.js, `<model-viewer>` (+ `model-viewer-meshopt.js`),
  `character-studio/`, `src/scene-studio/`, walk/tour companions.
...
8. **Layout stability:** reserve space for async content (models, images, embeds)
