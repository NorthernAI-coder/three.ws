# 🚀 Innovation Brief — {FEATURE NAME}

> **Task file:** `prompts/feature-innovation/{FILENAME}`
> **Surface:** `{ROUTE(S)}`
> **Primary source:** `{SOURCE FILES}`
> **Atlas reference:** `docs/ux-flows/{CLUSTER ATLAS FILE}` (current UX, traced step-by-step)
...
{1–2 paragraphs. Who is the end user and what are they trying to accomplish? What does this feature exist for? What does "gamechanging" mean *specifically* for this surface? Demand genuine invention — something users can't get elsewhere — not a clone of a competitor.}

## 2. The bar

Build the version that makes someone screenshot it and share it. Benchmark against the best in the world ({relevant comparators}). **If your result merely matches what already exists, you have failed this brief.** Invent something true and new that creates real, obvious value for the user.

## 3. Current state — ground truth (verify in source before you touch anything)

- **Route(s):** {routes}
- **Source:** {files}
- **Current flow:** {N} steps — {one-line summary of today's path}
- **What works today:** {bullet}
- **Real APIs / dependencies already wired:** {endpoints, services, on-chain}
- **Where it's mediocre, thin, or unfinished:** {honest assessment — this is where the value is}

## 4. Innovation directions (seeds — you are expected to go beyond them)

- {bold, specific idea 1}
- {bold, specific idea 2}
- {bold, specific idea 3}
...
4. **Delete this task file** — `prompts/feature-innovation/{FILENAME}`.
...
- **This feature's current UX:** `docs/ux-flows/{CLUSTER ATLAS FILE}`
