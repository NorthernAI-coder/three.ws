...
Last updated: 2026-07-08 (full 04–21 verification pass + P0 production-incident fix — see below). Re-run prompt 14 near the end to refresh status.

**2026-07-08 P0 incident (now fixed, redeploying):** every POST/JSON-RPC endpoint on production (`/api/mcp-studio`, `/api/mcp`, `/api/mcp-3d`, `/api/3d/studio`, `/api/forge`, and ~380 more via the shared `readJson`/`readBody` helper) was hanging indefinitely with zero response — not a rate-limit 429, a true hang. Root cause: the Cloud Run migration's `server/index.mjs` runs `express.json()`/etc. ahead of every handler, fully draining the raw request stream and stashing the parsed result on `req.body` — but `api/_lib/http.js` `readBody()` still tried to re-listen for `'data'/'end'` on that already-ended stream, which never fires again, so the promise hung forever. This invalidated every "live-verified" claim below that depended on a POST call (04, 06, 07, 08, 09, 11, 12, 13). Fixed in `api/_lib/http.js` `readBody()` (prefers `req.rawBody`/`req.body`, falls back to the raw stream with a `req.complete` guard + 15s timeout) and `server/index.mjs` (captures `req.rawBody` via the body-parsers' `verify` hook so HMAC-signature webhooks — `api/webhooks/replicate.js`, QStash, SNS — still see exact bytes). A second, unrelated deploy-blocker was found and fixed while shipping this: `package-lock.json` was out of sync with `package.json`'s `packages/defi-utils` + `packages/tool-sdk` workspaces, so `npm ci` (every Cloud Build deploy) failed outright — synced and committed. Redeployed via `npm run build:gcp`-equivalent + `gcloud builds submit` from a clean `git archive HEAD` (the live worktree has ~100 unrelated concurrent-agent files in flight, so the deploy source was isolated to committed HEAD only). Re-verify every "live" claim below against production after this deploy lands.
...
| **OpenAI App Directory** | blocked | 04–06 | Package + audit done (`_generated/openai-submission.md`, 7/7 policy PASS; 3 real-model screenshots). **B1 root cause found + fixed 2026-07-08:** not a quota issue — the shared `readBody()` helper hung forever on every POST once the Cloud Run migration pre-drained the stream (see top-of-file P0 note); fix committed, deploy in flight. **B2:** `/viewer?src=` 404 — `dist/viewer.html` exists and `vercel.json`'s `/viewer/? → /viewer.html` rewrite is correct, so this may just clear once the fresh deploy lands; re-curl after redeploy before assuming further work is needed. `[HUMAN: OpenAI identity verification + support contact + final submit]` | — |
...
| 09 | Conversational remixable 3D | 🟨 complete; one real test regression found+fixed | `refine_model` (`mcp-server/src/tools/refine-model.js` paid, `api/_mcp-studio/tools.js` free), shared lineage core (`mcp-server/src/tools/_lineage.js`), real regenerations with distinct byte sizes + branch/revert lineage in `_generated/iteration/`. Royalty split math + caps unit-tested and correct (`api/_lib/remix-royalty.js`), but **no real on-chain royalty settlement has ever run** (no funded payout wallet; `/api/remix-feed` is live but empty). Found+fixed: `tests/mcp-tool-result.test.js` had a stale `writes[]` allowlist missing `refine_model` (real regression, now fixed) |
| 10 | Claude plugin marketplace | ✅ complete (prior session) | 4 installable plugins |
| 11 | Agent Skills pack | ✅ complete | `.agents/skills/` — 40 skills incl. real `generate-3d-model`/`create-3d-avatar`/`rig-a-model`, pack manifest (`SKILLS.md`, `skills-pack.json`), `docs/agent-skills.md` (3 install paths), category tags, grep-clean 3d/creative subset. Live E2E test against `/api/mcp-studio` was blocked by the P0 hang (now fixed) |
| 12 | OpenAI GPT Store Actions | 🟨 built + previously live-verified, was blocked by the P0 hang | `api/3d/studio.js` + `_generated/openai-actions.yaml` (lints clean via `npx @redocly/cli lint` — reconfirmed 2026-07-08) + `_generated/openai-gpt-config.md`. A captured real production response (2026-07-07) shows a working GLB still resolving live. Live re-verification blocked by the P0 hang until redeploy lands |
| 13 | MCP registries & directories | ✅ substantially complete, republish staged | 7 root `server*.json` + `mcp-server/server.json` all schema-current; canonical `_generated/mcp-listing-source.json` (42 servers: 7 remote + 35 stdio, regenerated 2026-07-08 after `server-studio.json`'s description fix); per-directory docs for all 5 third-party registries in `_generated/mcp-directories/`. Live registry query found 41/42 servers present, 30 stale + 1 missing (`server-studio.json`) — `_generated/mcp-registry-republish.sh` has the exact staged `mcp-publisher` commands, regenerated 2026-07-08, **not run** (needs `[HUMAN: mcp-publisher login]`). `STRUCTURE.md`'s stale "38 total" corrected to 42 |
...
| 17 | Embodied on-chain identity | ⬜ not started (verified 2026-07-08) | No `persona_identity`/`persona_tip`/`persona_send` code anywhere. Not human-blocked — genuinely unbuilt, needs net-new implementation. Reusable infra confirmed real: `provision_wallet` (`api/_mcpagent/tools.js:345`), `solana_agent_reputation` (`api/_mcp/tools/solana.js:109`), persona storage (prompt 07) |
| 18 | Token-gated 3D embeds | ⬜ not started (verified 2026-07-08) | No `create_gated_embed` code anywhere. Not human-blocked — genuinely unbuilt. Strong reusable starting point found: `api/_lib/world-gate.js` + `api/_lib/holder-pass.js` (server-verified SPL balance + signed short-lived access token, already shipped for multiplayer world gating — same pattern this prompt needs, just not wired to `src/avatar-embed.js`) |
| 19 | Verifiable AI-3D provenance | ⬜ not started (verified 2026-07-08) | No `specs/PROVENANCE_3D.md`, `anchor_provenance`, or `verify_provenance` anywhere. Not human-blocked — genuinely unbuilt. Reusable infra confirmed real: `api/_lib/attest-event.js` (existing Solana memo-attestation pattern used by `review-attest.js`/`tournament-attest.js`), sha256 hashing helpers already used platform-wide |
| 20 | Spatial MCP standard | ⬜ not started (verified 2026-07-08) | No `specs/SPATIAL_MCP.md`, `validate_spatial_response`, or `docs/spatial-mcp.md` anywhere. Not human-blocked — genuinely unbuilt (spec-writing + validator + wiring existing 3D tool outputs to a shared shape) |
| 21 | AR-ready exports | ⬜ not started (verified 2026-07-08) | No `export_ar` tool anywhere. Not human-blocked — genuinely unbuilt, but real reusable AR infra already exists and is coin-clean: `src/usdz-pipeline.js`/`src/usdz-animated.js` (real in-browser GLB→USDZ via three.js `USDZExporter`), `src/ar/quick-look.js` + `src/ar/scene-viewer.js` (real device-aware launch), all currently scoped to avatars only, not exposed as a generic MCP tool |
...
8. **Free-lane re-verification before filing** — RESOLVED (root cause, not just symptom): the hang was not a NIM-timeout — it was the platform-wide `readBody()` P0 bug described at the top of this file, affecting every POST/JSON-RPC endpoint. Fixed and redeploying as of 2026-07-08. Still re-run the free smoke test immediately before submitting either Claude listing to confirm the redeploy is live and the free lane returns a real GLB in a reasonable time.
