# Fix failing tests — `tests/mcp-auth-challenge.test.js`

> 1 of 6 tests in this file are failing. Make it green without weakening coverage.

## Reproduce
```bash
npx vitest run tests/mcp-auth-challenge.test.js
```

## Failing tests (1)

### sendAuthChallenge status negotiation › always ships the x402 envelope and discovery headers, on both statuses
```
AssertionError: expected undefined to be defined
    at /workspaces/three.ws/tests/mcp-auth-challenge.test.js:86:33
```
...
## Done when
- [ ] `npx vitest run tests/mcp-auth-challenge.test.js` is fully green.
