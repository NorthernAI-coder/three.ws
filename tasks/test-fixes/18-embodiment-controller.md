# Fix failing tests — `tests/embodiment-controller.test.js`

> This test file fails to load/run at all. Make it green without weakening coverage.

## Reproduce
```bash
npx vitest run tests/embodiment-controller.test.js
```
...
```
Cannot find module '../src/embodiment/controller.js' imported from /workspaces/three.ws/tests/embodiment-controller.test.js
```
## What to do
...
## Done when
- [ ] `npx vitest run tests/embodiment-controller.test.js` is fully green.
