# Fix failing tests — `tests/metering.test.js`

> 1 of 11 tests in this file are failing. Make it green without weakening coverage.

## Reproduce
```bash
npx vitest run tests/metering.test.js
```

## Failing tests (1)

### AWS Marketplace — MeterUsage call shape (SDK boundary mocked) › issues a MeterUsage command with the product code, dimension, and idempotency allocation
```
TypeError: __vite_ssr_import_0__.vi.dontMock is not a function
    at /workspaces/three.ws/tests/metering.test.js:194:6
...
## Done when
- [ ] `npx vitest run tests/metering.test.js` is fully green.
