# Fix failing tests — `tests/agora-humans.test.js`

> This test file fails to load/run at all. Make it green without weakening coverage.

## Reproduce
```bash
npx vitest run tests/agora-humans.test.js
```
...
```
Failed to resolve entry for package "@three-ws/solana-agent". The package may have incorrect main/module/exports specified in its package.json.
```
## What to do
...
## Done when
- [ ] `npx vitest run tests/agora-humans.test.js` is fully green.
