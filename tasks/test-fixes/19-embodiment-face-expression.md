# Fix failing tests — `tests/embodiment-face-expression.test.js`

> This test file fails to load/run at all. Make it green without weakening coverage.

## Reproduce
```bash
npx vitest run tests/embodiment-face-expression.test.js
```
...
```
Cannot find module '../src/embodiment/face-expression.js' imported from /workspaces/three.ws/tests/embodiment-face-expression.test.js
```
## What to do
...
## Done when
- [ ] `npx vitest run tests/embodiment-face-expression.test.js` is fully green.
