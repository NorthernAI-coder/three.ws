# Fix failing tests — `tests/api/security-csrf-gates.test.js`

> This test file fails to load/run at all. Make it green without weakening coverage.

## Reproduce
```bash
npx vitest run tests/api/security-csrf-gates.test.js
```
...
```
Failed to resolve entry for package "@nirholas/pump-sdk". The package may have incorrect main/module/exports specified in its package.json.
```
## What to do
...
## Done when
- [ ] `npx vitest run tests/api/security-csrf-gates.test.js` is fully green.
