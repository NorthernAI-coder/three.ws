# Fix failing tests — `tests/api/agent-subscription-tiers.test.js`

> 2 of 24 tests in this file are failing. Make it green without weakening coverage.

## Reproduce
```bash
npx vitest run tests/api/agent-subscription-tiers.test.js
```
...
### routing guards › 400s on a non-UUID tier id
```
...
If you need to partially mock a module, you can use "importOriginal" helper inside:
    ...
    at /workspaces/three.ws/tests/api/agent-subscription-tiers.test.js:124:28
```

### POST /api/agents/:id/tiers › 400s when the price is below the $0.99 minimum
```
Error: [vitest] No "isDbUnavailableError" export is defined on the "../../api/_lib/db.js" mock. Did you forget to return it from "vi.mock"?
If you need to partially mock a module, you can use "importOriginal" helper inside:
    ...
    at processTicksAndRejections (node:internal/process/task_queues:104:5)
    at Module.invoke (/workspaces/three.ws/tests/_helpers/monetization.js:69:2)
    at /workspaces/three.ws/tests/api/agent-subscription-tiers.test.js:195:33
```
...
## Done when
- [ ] `npx vitest run tests/api/agent-subscription-tiers.test.js` is fully green.
