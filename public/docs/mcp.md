...
### Hosted remote servers (Streamable HTTP, no install)

These carry your three.ws account context (OAuth) — use them for anything tied to your avatars, wallet, or published services.
...
| Agent wallet | `/api/mcp-agent` | The agent's custodial wallet: balance, find + **pay** services, and **`monetize_endpoint`** — put a price on an API you serve and earn USDC |
...
Add a hosted server to Claude Code in one line (OAuth opens in your browser on first use):

```bash
claude mcp add --transport http three-ws       https://three.ws/api/mcp
claude mcp add --transport http three-ws-studio https://three.ws/api/mcp-3d
claude mcp add --transport http three-ws-wallet https://three.ws/api/mcp-agent
claude mcp add --transport http three-ws-bazaar https://three.ws/api/mcp-bazaar
```

### Install-and-run servers (npm, stdio, one line)

Each runs locally with `npx` — no account needed unless a tool spends money or writes to your account. Grouped by what they do:
...
```bash
npx -y @three-ws/x402-mcp            # a self-custodial wallet: find, inspect & PAY any x402 service in USDC
...
- **Buy** — `@three-ws/x402-mcp` gives any agent a wallet that pays for services from a Solana key you control. Its `pay_and_call` settles **any** three.ws paid endpoint generically — the paid vanity grind (`/api/x402/vanity`), symbol-availability checks, and anything else in the bazaar — so there's no per-endpoint package to install.
- **Sell** — the hosted `/api/mcp-agent` server's `monetize_endpoint` puts a price on an API you already serve; three.ws hosts the paywall, settles each buyer's USDC to your wallet, and lists it in the bazaar. (This one is account-bound, so it lives on the hosted OAuth server, not an npm package.)
