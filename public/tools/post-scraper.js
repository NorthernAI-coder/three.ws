/*
 * three.ws — Post Scraper console panel
 ...
 *
 * Launch:  load this file in the page (see /tools/scraper) or paste its
 *          contents into DevTools. Re-running toggles the panel.
 *
 * Everything lives under window.__threewsScraper so a second run can find the
...
(function () {
  ...
  if (window.__threewsScraper && window.__threewsScraper.toggle) {
    window.__threewsScraper.toggle();
    return;
  }

  var SETTINGS_KEY = 'three.ws:scraper:settings:v1';
  var PANEL_ID = 'threews-scraper-panel';
  ...
  function buildPanel() {
    ...
    var header = el('div', { class: 'tws-hd' }, [
      ...
      el('span', { class: 'tws-src', text: 'three.ws' }),
      collapseBtn,
      closeBtn
    ]);

    var panel = el('div', { id: PANEL_ID, role: 'dialog', 'aria-label': 'three.ws post scraper' }, [header, body]);
