# Spend Reservation Leak Detector
...
Implement the **Spend Reservation Leak Detector** autonomous pipeline that calls `/api/_lib/agent-trade-guards (DB query)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/_lib/agent-trade-guards (DB query)`
- **Price:** `free (DB)` per call
- **Category:** Finance

## What to Build

Queries x402_spend_reservations for entries older than 1 hour that were never finalized. These represent leaked reservations where releaseSpendReservation was never called. Cleans them up and alerts.
