# 051: Token Intel: $THREE Price & Holder Snapshot

## Objective
Build a real, wired x402 use case that calls the `/api/x402/token-intel` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.

## x402 Endpoint
- **Path:** `/api/x402/token-intel`
- **Method:** POST
- **Body:** `{"mint":"FeMbDoX7R1Psc4GEcvJdsbNbZA3bfztcyDCatJVJpump","network":"mainnet"}`

## What to Build
Pay x402 to monitor $THREE token: price_usd, holder_count, top_holder_pct, 24h volume. Extract all fields into oracle_intel_signals topic=three_price. Run every 15 minutes. If price drops >10% in 24h, signal=bearish.
