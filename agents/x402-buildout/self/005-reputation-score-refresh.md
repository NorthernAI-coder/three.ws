# Agent Reputation Score Refresh
...
Implement the **Agent Reputation Score Refresh** autonomous pipeline that calls `/api/mcp (tool: solana_reputation)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/mcp (tool: solana_reputation)`
- **Price:** `$0.001 USDC/call` per call
- **Category:** MCP Health

## What to Build

Polls solana_reputation for every registered agent address in the DB. Updates reputation_score column. Agents whose score drops below threshold get flagged. Powers the leaderboard and trust badges on agent profiles.
