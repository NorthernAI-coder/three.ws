# Cross-Chain Payment Cost Comparison
...
Implement the **Cross-Chain Payment Cost Comparison** autonomous pipeline that calls `/api/x402-pay (Base + Solana)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/x402-pay (Base + Solana)`
- **Price:** `$0.001 USDC/call` per call
- **Category:** Finance

## What to Build

Makes identical $0.001 payments on Solana and Base and records total cost including gas. Tracks the gas premium ratio over time. Used to recommend the cheapest network to users and adjust default pricing.
