# Payment Proof Idempotency Audit
...
Implement the **Payment Proof Idempotency Audit** autonomous pipeline that calls `/api/x402-pay (same proof twice)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/x402-pay (same proof twice)`
- **Price:** `$0.001 USDC/call` per call
- **Category:** Security

## What to Build

Submits the same payment proof twice in rapid succession to verify the idempotency store correctly rejects the second. Confirms no double-settlement. Runs daily. Critical anti-fraud check.
