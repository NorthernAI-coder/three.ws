# Sniper Agent Data Enrichment via crypto-intel
...
Implement the **Sniper Agent Data Enrichment via crypto-intel** autonomous pipeline that calls `/api/x402/crypto-intel` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/x402/crypto-intel`
- **Price:** `$0.01 USDC/call` per call
- **Category:** Trading

## What to Build

Enriches agent-sniper oracle_conviction scores by calling crypto-intel for every coin the sniper is actively watching. Feeds headline sentiment + signal + confidence into the oracle gate. The most critical self-loop: our trading engine pays our intelligence API to make better decisions.
