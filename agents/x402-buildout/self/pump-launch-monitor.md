# 046: Pump Launch Monitor: Recent Launches

## Objective
Build a real, wired x402 use case that calls the `/api/x402/pump-agent-audit` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.

## x402 Endpoint
- **Path:** `/api/x402/pump-agent-audit`
- **Method:** POST
- **Body:** `{"limit":10,"sort":"newest"}`

## What to Build
Pay x402 to audit the 10 most recently launched pump.fun tokens. Extract { count, newest_mint, newest_name, avg_initial_liquidity }. Wire into sniper pipeline — new launches with high initial liquidity are snipe candidates.
