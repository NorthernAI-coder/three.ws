# Solana Agent Registration Health Check
...
Implement the **Solana Agent Registration Health Check** autonomous pipeline that calls `/api/mcp (tool: solana_register)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/mcp (tool: solana_register)`
- **Price:** `$0.001 USDC/call` per call
- **Category:** MCP Health

## What to Build

Registers a canary agent identity on Solana to verify the solana_register tool is functioning end-to-end. Checks the resulting on-chain record exists. Alerts on failure. Canary registration is a known test agent key.
