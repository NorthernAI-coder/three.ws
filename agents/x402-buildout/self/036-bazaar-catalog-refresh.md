...
Implement the **Bazaar Service Catalog Daily Refresh** autonomous pipeline that calls `/api/mcp-bazaar (bazaar_search_services + bazaar_service_details)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/mcp-bazaar (bazaar_search_services + bazaar_service_details)`
- **Price:** `$0.001 USDC/call` per call
- **Category:** Discovery

## What to Build

Paginates through all services on the bazaar daily. Stores snapshot. Computes diff from previous day. Detects new services (opportunity alerts), removed services (pipeline dependency alerts), and price changes.
