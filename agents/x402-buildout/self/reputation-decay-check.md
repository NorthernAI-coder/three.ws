# 070: Reputation: Score Decay Monitor

## Objective
Build a real, wired x402 use case that calls the `/api/x402/agent-reputation` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.
...
## What to Build
Pay x402 to get a report of agents whose reputation score has decayed by >10 points in 7 days. Extract { decayed_count, fastest_decline_agent, avg_decay }. Decay spikes could indicate coordinated abuse or score manipulation.
