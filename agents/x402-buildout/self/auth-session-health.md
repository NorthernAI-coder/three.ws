# 083: Auth: Session Lifecycle Health

## Objective
Build a real, wired x402 use case that calls the `/api/x402/auth-health` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.
...
## What to Build
Pay x402 to exercise the auth session lifecycle: create, validate, refresh, expire. Extract { all_pass: bool, failed_step, latency_ms }. Runs every 30 minutes as a security-critical health check.
