...
Implement the **API Key Bypass Security Test** autonomous pipeline that calls `/api/x402/* with X-API-Key header` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/x402/* with X-API-Key header`
- **Price:** `free (bypass)` per call
- **Category:** Security

## What to Build

Tests the API key bypass lane with valid and invalid keys. Confirms valid key grants access without payment. Confirms invalid key requires payment. Detects any bypass vulnerabilities introduced by refactors.
