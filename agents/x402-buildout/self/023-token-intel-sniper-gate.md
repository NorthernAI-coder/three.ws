...
Implement the **Token Intel Pre-Snipe Gate** autonomous pipeline that calls `/api/x402/token-intel` as part of the x402 autonomous spend loop.
...
Before the sniper claims a new mint, calls token-intel to get on-chain stats (holder count, top 10 concentration, dev wallet %). Adds a rugpull-risk sub-score to the oracle conviction. High-risk mints are auto-rejected.
