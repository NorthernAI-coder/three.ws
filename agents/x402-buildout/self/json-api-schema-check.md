# 082: JSON API: Schema Conformance Check

## Objective
Build a real, wired x402 use case that calls the `/api/x402/schema-check` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.
...
## What to Build
Pay x402 to hit the public changelog JSON API and validate the response schema matches the spec. Extract { valid: bool, version, entry_count }. Catches breaking schema changes before users notice.
