# 072: Forge: Content Generation Health

## Objective
Build a real, wired x402 use case that calls the `/api/x402/forge` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.
...
## What to Build
Pay x402 to run a minimal forge content generation request. Verify it completes within 5 seconds. Extract { generated: bool, latency_ms, token_count }. If latency > 5000ms, log a forge performance alert.
