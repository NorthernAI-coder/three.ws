...
Implement the **MCP Model Validation Sweep** autonomous pipeline that calls `/api/mcp (tool: inspect_model)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/mcp (tool: inspect_model)`
- **Price:** `$0.001 USDC/call` per call
- **Category:** MCP Health

## What to Build

Scheduled sweep that calls inspect_model for every public GLB in the database. Identifies geometry errors, missing bones, and unsupported features before users encounter them. Builds a quality score time-series per model.
