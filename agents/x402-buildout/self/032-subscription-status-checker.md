# Subscription Status Health Check
...
Implement the **Subscription Status Health Check** autonomous pipeline that calls `/api/x402/admin/subscriptions` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/x402/admin/subscriptions`
- **Price:** `free (internal)` per call
- **Category:** Commerce

## What to Build

Enumerates all active x402 subscriptions and verifies they are still valid (not expired, key not rotated). Emails subscribers 7 days before expiry. Prevents surprise service interruptions.
