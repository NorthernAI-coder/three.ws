# IBM Granite Inference Health Check
...
Implement the **IBM Granite Inference Health Check** autonomous pipeline that calls `/api/ibm-mcp (all 5 Granite tools)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/ibm-mcp (all 5 Granite tools)`
- **Price:** `$0.001 USDC/call` per call
- **Category:** MCP Health

## What to Build

Calls each IBM Granite tool with a canary prompt (e.g., "health check" classification query). Verifies the watsonx backend is responding and responses match expected schema. Tracks token throughput.
