...
Implement the **Animation Retargeting QA** autonomous pipeline that calls `/api/x402/animation-download` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/x402/animation-download`
- **Price:** `$0.005 USDC/call` per call
- **Category:** 3D Pipeline

## What to Build

Downloads retargeted idle/walk clips for a representative set of rig types (Mixamo, VRM, Avaturn, Daz). Validates that all 6 canonical clips are present and non-zero bytes. Catches glb-canonicalize.js regressions.
