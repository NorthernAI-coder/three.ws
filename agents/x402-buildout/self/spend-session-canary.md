# 065: Spend Session: Canary Create+Consume

## Objective
Build a real, wired x402 use case that calls the `/api/x402/spend-session` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.
...
## What to Build
Pay x402 to create a minimal spend session, consume it immediately, and verify the cycle completes cleanly. Extract { created: bool, consumed: bool, latency_ms }. This is the most important health check for the x402 governance layer.
