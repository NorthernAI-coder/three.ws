# 074: Rate Limit: Threshold Probe

## Objective
Build a real, wired x402 use case that calls the `/api/x402/rate-limit-probe` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.
...
## What to Build
Pay x402 to get the current rate limit status for the crypto-intel endpoint. Extract { remaining_calls, reset_at, limit }. Use to throttle the autonomous loop dynamically based on available capacity.
