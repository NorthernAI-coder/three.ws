# 059: Bazaar: New Listing Feed

## Objective
Build a real, wired x402 use case that calls the `/api/x402/bazaar-feed` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.
...
## What to Build
Pay x402 to retrieve the 10 newest bazaar listings. Extract { count, newest_id, newest_price, categories }. Run every 30 minutes to keep the bazaar catalog warm and detect listing spikes that indicate agent marketing activity.
