...
Implement the **Charity Split Audit** autonomous pipeline that calls `/api/x402-merchant (charity split flow)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/x402-merchant (charity split flow)`
- **Price:** `$0.01 USDC/call` per call
- **Category:** Finance

## What to Build

Makes payments to a merchant with charity_split configured. Verifies the split amount is correctly routed to the charity address on-chain. Runs weekly. Ensures donation promises are kept.
