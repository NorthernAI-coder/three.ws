# 075: API Key: Validity Health Check

## Objective
Build a real, wired x402 use case that calls the `/api/x402/api-key-health` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.
...
## What to Build
Pay x402 to verify the autonomous loop API key is valid and has the necessary scopes. Extract { valid: bool, scopes: [], expires_at }. Run hourly. Alert 24h before expiry.
