# 068: DID: Registry Health Sweep

## Objective
Build a real, wired x402 use case that calls the `/api/x402/did` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.
...
## What to Build
Pay x402 to sweep the DID registry for the 10 most recently created DIDs. Verify resolution works for each. Extract { count, resolvable_count, failed_count }. Alert if failed_count > 0.
