...
Implement the **Rig Complexity Scorer** autonomous pipeline that calls `/api/mcp (inspect_model)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/mcp (inspect_model)`
- **Price:** `$0.001 USDC/call` per call
- **Category:** 3D Pipeline

## What to Build

Computes a complexity score (bone count, vertex count, texture size) for every avatar using inspect_model data. Used to tier pricing in the marketplace and warn users about performance-heavy models.
