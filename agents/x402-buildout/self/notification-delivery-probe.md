# 079: Notification: Delivery Probe

## Objective
Build a real, wired x402 use case that calls the `/api/x402/notify` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.
...
## What to Build
Pay x402 to send a canary notification through the platform notification system. Verify delivery confirmation within 2 seconds. Extract { delivered: bool, channel, latency_ms }.
