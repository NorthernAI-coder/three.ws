...
Implement the **Avatar Optimization Pipeline** autonomous pipeline that calls `/api/mcp (tool: optimize_model)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/mcp (tool: optimize_model)`
- **Price:** `$0.001 USDC/call` per call
- **Category:** MCP Health

## What to Build

Nightly job that calls optimize_model on the top 50 most-viewed avatars, compressing geometry and textures. Results stored; users automatically served optimized GLBs. Measures savings in bytes and load time improvement.
