# Scene Capture Video Queue Processor
...
Implement the **Scene Capture Video Queue Processor** autonomous pipeline that calls `/api/x402/asset-download (video2scene)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/x402/asset-download (video2scene)`
- **Price:** `$0.01 USDC/call` per call
- **Category:** 3D Pipeline

## What to Build

Monitors the pending scene capture queue and submits queued videos to the GPU worker. Polls for completion, stores .ply result. Auto-processes user-uploaded videos without manual intervention.
