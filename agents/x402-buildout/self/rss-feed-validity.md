# 081: RSS Feed: Changelog XML Validity

## Objective
Build a real, wired x402 use case that calls the `/api/x402/feed-health` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.
...
## What to Build
Pay x402 to fetch and validate the changelog RSS feed. Verify it parses as valid XML, has correct item count, and the latest entry matches the DB. Extract { valid: bool, item_count, latest_title }.
