# 080: Telegram Bot: Changelog Bot Health

## Objective
Build a real, wired x402 use case that calls the `/api/x402/telegram-health` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.
...
## What to Build
Pay x402 to ping the changelog Telegram bot and verify it can reach the Telegram API. Extract { reachable: bool, bot_id, latency_ms }. If unreachable, alert ops — new changelog entries wont reach holders.
