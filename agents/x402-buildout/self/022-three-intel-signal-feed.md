...
Implement the **$THREE Signal Feed** autonomous pipeline that calls `/api/x402/three-intel` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/x402/three-intel`
- **Price:** `$0.01 USDC/call` per call
- **Category:** Trading

## What to Build

Calls three-intel every 15 minutes to get the latest $THREE market signal. Stores in a time-series table. Powers the $THREE price widget and informs $THREE-denominated x402 pricing adjustments.
