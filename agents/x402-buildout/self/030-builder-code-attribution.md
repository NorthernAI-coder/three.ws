# Builder Code Attribution Tracker
...
Implement the **Builder Code Attribution Tracker** autonomous pipeline that calls `/api/x402/* (check X-PAYMENT-RESPONSE)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/x402/* (check X-PAYMENT-RESPONSE)`
- **Price:** `$0.001 USDC/call` per call
- **Category:** Finance

## What to Build

Makes a payment and verifies the X-PAYMENT-RESPONSE header includes our builder code (three_d_agent). Ensures Coinbase builder rewards are credited. Alerts if attribution is missing from any endpoint.
