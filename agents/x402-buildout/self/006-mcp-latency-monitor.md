...
Implement the **MCP Tool Latency Monitor** autonomous pipeline that calls `/api/mcp (all tools)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/mcp (all tools)`
- **Price:** `$0.001 USDC/call` per call
- **Category:** Observability

## What to Build

Calls each MCP tool with a minimal valid input every 5 minutes. Records p50/p95/p99 latencies to x402_perf_log. Dashboard shows SLA health. Alerts if any tool exceeds 2s p95.
