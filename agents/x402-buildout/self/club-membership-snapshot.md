# 066: Club: Membership Snapshot

## Objective
Build a real, wired x402 use case that calls the `/api/x402/club-cover` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.
...
## What to Build
Pay club cover to take a membership snapshot for the three_holders club. Extract { member_count, active_last_7d, new_this_week }. Monitor for membership growth or churn. Run daily.
