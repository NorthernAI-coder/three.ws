# 054: Pay-By-Name: Known Wallet Resolution

## Objective
Build a real, wired x402 use case that calls the `/api/x402/pay-by-name` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.
...
## What to Build
Pay x402 to resolve three.ws platform wallet name to on-chain address. Validates the pay-by-name registry is functioning. Extract { name, address, verified }. Run every 10 minutes as health check.
