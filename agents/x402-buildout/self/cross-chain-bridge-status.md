# 078: Cross-Chain: Bridge Status Monitor

## Objective
Build a real, wired x402 use case that calls the `/api/x402/cross-chain` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.
...
## What to Build
Pay x402 to get current bridge status between Solana and other chains. Extract { bridges: [{chain, status, latency_ms}] }. Any bridge with status=down is logged as a platform risk.
