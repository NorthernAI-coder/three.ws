# External x402 Service Uptime Monitor
...
Implement the **External x402 Service Uptime Monitor** autonomous pipeline that calls `registered external services (HEAD/OPTIONS probe)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `registered external services (HEAD/OPTIONS probe)`
- **Price:** `free (probe)` per call
- **Category:** Reliability

## What to Build

Probes every external x402 service in our registry with a HEAD request to check it returns 402 (meaning it is live). Flags services that return 5xx or timeout. Prevents us from calling dead endpoints in production pipelines.
