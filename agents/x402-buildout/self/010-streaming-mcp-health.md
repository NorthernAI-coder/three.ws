...
Implement the **SSE Streaming MCP Health** autonomous pipeline that calls `/api/mcp (SSE mode)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/mcp (SSE mode)`
- **Price:** `$0.001 USDC/call` per call
- **Category:** Infrastructure

## What to Build

Opens an SSE stream to /api/mcp, pays for a tool call, and verifies the streaming response arrives token-by-token before close. Detects dropped connections, stalled streams, or broken chunked encoding.
