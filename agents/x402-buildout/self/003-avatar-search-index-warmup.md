...
Implement the **Avatar Search Index Warmup** autonomous pipeline that calls `/api/mcp (tool: search_public_avatars)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/mcp (tool: search_public_avatars)`
- **Price:** `$0.001 USDC/call` per call
- **Category:** MCP Health

## What to Build

Fires 20 common search queries (human, robot, anime, warrior, etc.) against search_public_avatars on cold start. Pre-warms any caches and validates the full search pipeline returns ranked results with thumbnails.
