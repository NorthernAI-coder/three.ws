# 048: Pump Volume Anomaly Detection

## Objective
Build a real, wired x402 use case that calls the `/api/x402/crypto-intel` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.
...
## What to Build
Call crypto-intel with pump_volume_anomaly topic to detect abnormal trading volume on pump.fun in the last hour. Extract { anomaly: bool, mint, volume_ratio }. If anomaly is true and volume_ratio > 5, log as high-conviction oracle signal.
