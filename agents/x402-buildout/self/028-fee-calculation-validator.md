...
Implement the **Fee Calculation Validator** autonomous pipeline that calls `/api/x402/* (various amounts)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/x402/* (various amounts)`
- **Price:** `varies` per call
- **Category:** Finance

## What to Build

Makes calls at boundary amounts (1, 999, 1000, 1001, 999999 atomics) to verify fee math is correct across all pricing tiers. Detects off-by-one errors in atomic→decimal conversions.
