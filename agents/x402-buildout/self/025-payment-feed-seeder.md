...
Implement the **Live Payment Feed Seeder** autonomous pipeline that calls `/api/x402-pay (demo flow)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/x402-pay (demo flow)`
- **Price:** `$0.001 USDC/call` per call
- **Category:** Trading

## What to Build

Makes structured demo payments on a rotation to keep the live payment feed on the homepage populated with recent activity. Variety of topics, amounts, networks. Proves live system health to visitors.
