# 076: LLM Proxy: Latency Benchmark

## Objective
Build a real, wired x402 use case that calls the `/api/x402/llm-proxy` endpoint on three.ws, pays with the autonomous loop seed wallet, records the result to `x402_autonomous_log`, and extracts actionable signal data.
...
## What to Build
Pay x402 to send a minimal prompt through the LLM proxy and measure response latency. Extract { latency_ms, tokens_used, model }. Run every 10 minutes. Alert if p95 latency exceeds 3 seconds.
