...
Implement the **GLB Canonicalization Pipeline** autonomous pipeline that calls `/api/x402/model-check` as part of the x402 autonomous spend loop.
...
Runs every newly uploaded GLB through model-check to validate bone names, rig type detection, and animation compatibility. Stores canonicalization result. Prevents T-pose fallback regressions from new rig types.
