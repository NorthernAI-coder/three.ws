...
Implement the **Bazaar Discovery Warmup** autonomous pipeline that calls `/api/mcp-bazaar (bazaar_search_services)` as part of the x402 autonomous spend loop.

## x402 Endpoint

- **Route:** `/api/mcp-bazaar (bazaar_search_services)`
- **Price:** `$0.001 USDC/call` per call
- **Category:** Discovery

## What to Build

Runs 15 category searches on bazaar_search_services (trading, llm, analytics, etc.) on startup. Validates results contain live endpoints with pricing. Records catalog snapshot for drift detection.
