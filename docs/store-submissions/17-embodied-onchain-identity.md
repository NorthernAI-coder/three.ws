...
- `$THREE` remains the only coin referenced; grep clean. `npm test` green; add tests for binding determinism, spend-cap enforcement, and key-never-leaked. Evidence to `docs/store-submissions/_generated/identity/`.
