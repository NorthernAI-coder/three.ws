...
- Drive a real end-to-end delegation: discover → reputation-rank → pay (real USDC on the existing facilitator) → get result → render the provenance card. Capture the transcript + settlement reference to `docs/store-submissions/_generated/commerce/`.
