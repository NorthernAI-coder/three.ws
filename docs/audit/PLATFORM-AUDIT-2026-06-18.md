...
### 4. Root `llms.txt` is empty (BROKEN)
Root `llms.txt` is 0 bytes. **On investigation** the real, served file is
`public/llms.txt` (67 KB, generated by `scripts/build-page-index.mjs`, routed in
`vercel.json`); the root file is a vestigial duplicate referenced by nothing. **Fix
applied:** removed the root file (populating it would create a divergent second copy).
...
### 7. API boundary hardening — investigated, already handled (no change)
- `api/wallet/balances.js`: the env-var name appears only in a deliberate `503
  not_configured` operator hint (name, not value) — matches the codebase-wide
  graceful-503 convention. Not a secret leak; left as-is.
- `api/agent-economy/transact.js`: recipient is already gated by
  `isValidPubkey(recipient)` before transfer. No gap. The audit-pass flag was stale.
