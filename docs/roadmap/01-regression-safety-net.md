...
> Paste into a fresh Claude Code chat in the three.ws repo. Follow CLAUDE.md and `docs/roadmap/00-README.md`. Use TodoWrite. This prompt is **purely additive test/infrastructure** — it must not change any product behavior.
...
## Tasks (additive only)
1. **Inventory the current gate.** Run each script in the 00-README gate; record which exist, pass, or are missing. Document in `docs/roadmap/_generated/01/gate-inventory.md`.
...
6. **One command.** Add `npm run gate` to `package.json` that runs the full sequence, and document it in `docs/roadmap/00-README.md` (replace the inline list with `npm run gate` if cleaner).
